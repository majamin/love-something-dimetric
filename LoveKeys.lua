...
KeysTable.SingleKeyTimingUpdate = function(dt, k, KeyInfo, Suppress)
  if KeyInfo.Held and not Suppress then
    ...
    if
      KeyInfo.RepeatLength > KeyInfo.Repeat.Delay
      and KeyInfo.Repeat.Delay ~= 0
      and KeyInfo.Repeat.Repeat ~= 0
    then
...
KeysTable.AliasUpdate = function()
  for k, KeyInfo in pairs(KeysTable.KeyInfo) do
    local ToAliasPress, ToAliasUnpress =
      KeysTable.GetSingleKeyAliasUpdateInfo(KeyInfo)
...
KeysTable.update = function(dt)
  for k, KeyInfo in pairs(KeysTable.KeyInfo) do
    ...
    KeysTable.SingleKeyTimingUpdate(
      dt,
      k,
      KeyInfo,
      ToAliasPress or ToAliasUnpress
    )
...
KeysTable.Alias = function(From, To, Type, Namespace)
  ...
  for _, Key in pairs(To) do
    ...
    for _, KeyRemoveFrom in pairs(ToRemoveFrom) do
      for Index, RemoveCandidate in
        pairs(KeysTable.KeyInfo[KeyRemoveFrom].Alias[Namespace].To)
      do
    ...
    local ToAliasPress, ToAliasUnpress =
      KeysTable.GetSingleKeyAliasUpdateInfo(KeysTable.KeyInfo[Key])
